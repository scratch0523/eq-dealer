...
document.addEventListener("DOMContentLoaded", function () {
  const imageList = document.querySelector("#imageList__model");
  const globalCarousel = document.querySelector(".sliider__banner__carousel__model");
  const arrowBtns = document.querySelectorAll(".slider__banner__model i");
  const zoomContainer = document.querySelector(".zoom-container__model");
  let zoomImage = document.getElementById("zoomImage__model");
  let currentId = "ImageModel_1";
  const firstCardWidth = globalCarousel.querySelector(".slider__banner__carousel__card__model").offsetWidth;
  let childElements = document.getElementById("imageList__model").getElementsByTagName("li");
  ...
  function handleCarouselButton(btn) {
    globalCarousel.scrollLeft += btn.id === "left__model" ? -firstCardWidth : firstCardWidth;
  }
